...
- Perlだけでなく、<ruby>PHP<rt>ピーエイチピー</rt></ruby>や<ruby>Ruby<rt>ルビー</rt></ruby>など他のプログラミング言語でもこの傾向があるように思います。
...
- 開発環境エミュレート方式（<ruby>Cygwin<rt>シグウィン</rt></ruby>、<ruby>msys2<rt>エムシスツー</rt></ruby>）
...
これ以降、下記のように入力コマンドを示す記載が頻繁に例示されます。すべての個所で共通となりますが、先頭の「<ruby>$<rt>ダラー</rt></ruby>」はプロンプト（コマンドが入力ができる状態であること）を意味するため、コマンドの一部ではありません。プロンプトは、入力しないようにしてください。
