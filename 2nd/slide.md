...
1. `Hello, Perl Entrance!` という文字列を出力するスクリプト `hello_perl.pl` を作成して下さい。
...
これも立派な条件分岐ですが、ここでの条件式は「信号が青であるか、青でないか」というものです。
...
このリストはスクリプトに書かれている通りの「要素の集合」でしかなく、スクリプトPerlが加えたり減らしたりすることができません。
