...
- 末尾にある `</タグ>` を終了タグ、閉じタグといいます。
    - 終了タグにはタグ名の前にスラッシュがついています。

- 内容をタグで囲うのが基本です。

- `<タグ>内容</タグ>` のように、「開始タグ」と「中身」と「終了タグ」をあわせたものを要素といいます。
...
これから書かれる文書が HTML5 の形式であることをブラウザに伝えるためのタグです。
...
___
### HTMLとプログラミング言語
HTMLはそのままでは静的（ページ内容が書き変わらないもの）ですが、Perl や JavaScript の力を借りることで書き換えが可能になります。

内容を書き換えることを「動的」といいます。

- Perl や PHP や Ruby や Java
    - 「送信ボタン」を押すなどして、ページの読み込みが発生したタイミングで、サーバが書き換えて表示する
